...
def predict_datapoint():
    ...
    else: #this is for single prediction,fill one by one value in form
        data = CustomData(
...
# this is for batch prediction
@app.route('/batch', methods=['GET','POST'])
def perform_batch_prediction():

    if request.method == 'GET':
        return render_template('batch.html')#upload full data
    
    else:

        file = request.files['csv_file'] #update the key to 'csv_file
        #Directory path,save batch prediction data
